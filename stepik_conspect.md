...
`CREATE TABLE <имя таблицы> (<имя столбца 1> <тип>, <имя столбца 2> <тип>) VALUES(<первая строка>),(<вторая строка>);` - запрос на создание таблицы (типы INT PRIMARY KEY AUTO*INCREMENT,VARCHAR(<кол-во символов>), INT, DECIMAL(<максимальное кол-во цифр>,<кол-во цифр после запятой>))
Новая таблица может быть создана на основе данных из другой таблицы. Для этого используется запрос SELECT, результирующая таблица которого и будет новой таблицей базы данных. ` CREATE TABLE имя*таблицы AS SELECT ... `=====================================================================================================`SELECT <_(all)>, <имя столбца1>, <имя столбца2> FROM <имя таблицы>` - запрос на выборку с таблицы
=====================================================================================================
**_математические функции_**: (ссыль на таблицу)
=====================================================================================================`IF(логическое*выражение, выражение_1, выражение_2)`- условный оператор вернет в1 или в2 
=====================================================================================================`WHERE`- Выборка данных по условию 
=====================================================================================================`BETWEEN`и`IN` - Оператор`BETWEEN`позволяет отобрать данные, относящиеся к некоторому интервалу, включая его границы.
пример:` WHERE <имя столбца> BETWEEN 5 AND 14; ```
=====================================================================================================`ORDER BY` - сортировка (`ASC`- по возрастанию |`DESC`- по убыванию)
пример:`ORDER BY author, amount DESC; `
=====================================================================================================
Оператор LIKE (Ссылка)
% - Любая строка, содержащая ноль или более символов
* - Любой одиночный символ  
=====================================================================================================
DISTINCT - выбор уникальных элементов столбца - аналог GROUP BY
пример:`SELECT DISTINCT author FROM book; `
=====================================================================================================
`SUM и COUNT` групповые функции
пример: `count(title) as Различных_книг, sum(amount) as Количество_экземпляров`
=====================================================================================================
`MIN()`, `MAX()` и `AVG()`, - групповые функции - вычисляют минимальное, максимальное и среднее значение элементов столбца, относящихся к группе.
=====================================================================================================
`ROUND()` - среднее значение по столбцу
=====================================================================================================
HAVING - В запросах с групповыми функциями размещается после оператора `GROUP BY`
пример: `... GROUP BY author HAVING SUM(price _ amount) > 5000`=====================================================================================================`WHERE price = (SELECT MIN(price) FROM book )` - Вложенный запрос в where
=====================================================================================================`IN`- во вложенном запросе`WHERE имя_столбца IN (вложенный запрос, возвращающий один столбец)`=====================================================================================================`ANY`и`ALL`во вложенном запросе |`ANY`будут включены все записи, для которых выражение со знаком отношения верно хотя бы для одного элемента результирующего запроса.`ALL`в результирующую таблицу будут включены все записи, для которых выражение со знаком отношения верно для всех элементов результирующего запроса.
=====================================================================================================
UPDATE - Запросы на обновление `UPDATE таблица SET поле = выражение, поле2 = выражение2,`
...
=====================================================================================================
DELETE - Запрос на удаление | `DELETE FROM таблица WHERE .... IN ();`
=====================================================================================================
`LIMIT <кол-во строк>` - Для ограничения вывода записей. Размещается после раздела `ORDER BY`.
=====================================================================================================
...
`DAY('2020-02-01') = 1` |`MONTH('2020-02-01') = 2` | `YEAR('2020-02-01') = 2020` - ыделить часть даты
=====================================================================================================
...
=====================================================================================================
`group by ст1, ст2, ст3` Группировка данных по нескольким столбцам
`having count(*) > 1` - счетчик
